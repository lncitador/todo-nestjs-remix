import { Dialog, Popover } from '@headlessui/react';
import { Equals, Bell } from 'phosphor-react';
import React from 'react';
import { Button } from '../components/Button';
import { SearchInput } from '../components/SearchInput';
import { useStore } from '../hooks/useStore';
import { Aside } from './Aside';
import { SideNav } from './SideNav';

export const Header: React.FC = () => {
  const date: Date = new Date();
  const year: number = date.getFullYear();
  const month: number = date.getMonth();
  const day: number = date.getDate();

  const todayDate = `${year}, ${monthName[month].slice(0, 3)} ${day
    .toString()
    .padStart(2, '0')}`;

  const [sideNavOpen, toggleSideNav] = useStore(
    (state) => state.useSideNavModal,
  );

  const [asideOpen, toggleAside] = useStore((state) => state.useAsideModal);
  const [, toggleNewTask] = useStore((state) => state.useNewTaskModal);

  return (
    <header className="items-center grid grid-cols-[1fr_auto_1fr] gap-4 md:gap-0 md:flex">
      <button onClick={toggleSideNav} className="block mr-6 xl:hidden">
        <Equals size={24} />
      </button>
      <Dialog
        className="relative z-10 text-xs text-slate-600 dark:text-slate-400 xl:text-base sm:text-sm xl:hidden"
        open={sideNavOpen}
        onClose={toggleSideNav}
      >
        <Dialog.Overlay className="fixed inset-0 bg-black opacity-10" />
        <Dialog.Panel className="fixed top-0 left-0">
          <SideNav />
        </Dialog.Panel>
      </Dialog>
      <SearchInput />
      <div className="text-center">
        <span className="block text-sm font-bold uppercase text-slate-600 dark:text-slate-200 xl:hidden">
          TO-DO LIST
        </span>
        <p>{todayDate}</p>
      </div>
      <div className="flex flex-1">
        <Popover className="relative grid ml-auto sm:mr-4 md:mr-6 place-items-center">
          <Popover.Button className="relative">
            <Bell className="text-xl md:text-2xl" weight="fill" />
          </Popover.Button>
          <Popover.Panel className="absolute right-0 p-3 mt-1 border rounded-md bg-slate-100 dark:bg-slate-800 top-full w-52 border-slate-300 dark:border-slate-700">
            <p>Nothing to show here.</p>
          </Popover.Panel>
        </Popover>
        <Button
          onClick={toggleNewTask}
          className="fixed z-10 shadow-lg sm:static bottom-3 right-3 sm:z-0 min-w-max shadow-slate-400 dark:shadow-slate-900 sm:shadow-transparent"
        >
          Add new task
        </Button>
        <button onClick={toggleAside} className="block xl:hidden">
          <img
            src="data:image/jpeg;base64,"
            alt="This is a picture of a cat"
            className="w-10 h-10 ml-4 rounded-full"
          />
        </button>
        <Dialog
          className="relative z-10 text-xs text-slate-600 dark:text-slate-400 xl:text-base sm:text-sm xl:hidden"
          open={asideOpen}
          onClose={toggleAside}
        >
          <Dialog.Overlay className="fixed inset-0 bg-black opacity-10" />
          <Dialog.Panel className="fixed top-0 right-0">
            <Aside />
          </Dialog.Panel>
        </Dialog>
      </div>
    </header>
  );
};

const monthName: string[] = [
  'January',
  'February',
  'March',
  'April',
  'May',
  'June',
  'July',
  'August',
  'September',
  'October',
  'November',
  'December',
];
